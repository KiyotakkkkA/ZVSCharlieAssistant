...
export interface VectorStoreConfig { id: number; name: string; description: string; embeddingModelId: number | null; status: VectorStoreStatus; searchMode: VectorSearchMode; chunkSizeTokens: number; chunkOverlapTokens: number; vectorDimension: number | null; createdAt: string; updatedAt: string }
export interface VectorStoreDocument { id: number; vectorStoreId: number; fileName: string; mimeType: string; size: number; status: VectorDocumentStatus; progress: number; chunkCount: number; createdAt: string; errorMessage: string | null }
