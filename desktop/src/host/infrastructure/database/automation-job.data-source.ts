...
export class AutomationJobDataSource {
  ...
  enqueue(kind: AutomationJob["kind"], key: string, payload: unknown, priority = 0): void {
    this.db.prepare(
  ...
  leaseNext(workerId: string): AutomationJob | undefined {
    return this.db.transaction(() => {
      ...
      ).get() as { id: number; kind: AutomationJob["kind"]; payload_json: string; attempt: number; max_attempts: number } | undefined;
  ...
  updatePayload(id: number, payload: unknown): void {
    this.db.prepare("UPDATE automation_jobs SET payload_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=?")
  ...
  fail(job: AutomationJob, error: string): void {
    ...
    ).run(terminal ? "failed" : "queued", error, `+${Math.min(300, 2 ** job.attempt)} seconds`, job.id);
  }
}
