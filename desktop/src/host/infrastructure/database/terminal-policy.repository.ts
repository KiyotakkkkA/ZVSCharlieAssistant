...
import type { TerminalPolicy } from "../../../shared/models/terminal";
...
export class TerminalPolicyRepository {
  ...
  upsert(input: UpsertTerminalPolicyInput): TerminalPolicy {
    if (
      ...
      input.maxConcurrentSessions > 16
    )
      throw new Error("Некорректный лимит параллельных сессий");
    if (
      !Number.isInteger(input.defaultTimeoutSeconds) ||
      input.defaultTimeoutSeconds < 1 ||
      input.defaultTimeoutSeconds > 3600
    )
      throw new Error("Некорректный таймаут по умолчанию");
    if (
      !Number.isInteger(input.maxTimeoutSeconds) ||
      input.maxTimeoutSeconds < input.defaultTimeoutSeconds ||
      input.maxTimeoutSeconds > 86400
    )
      throw new Error(
    ...
    const allowedCommands = [
      ...new Set(
        input.allowedCommands.map((item) => item.trim()).filter(Boolean),
      ),
    ];
    for (const command of allowedCommands)
      if (
        !/^[A-Za-z]+-[A-Za-z]+$/.test(command) ||
        forbidden.has(command.toLowerCase())
      )
        throw new Error(
          `Команда ${command} не может быть добавлена в политику`,
        );
    this.database
      .prepare(
  ...
  createPendingSession(input: PendingTerminalSessionInput): void {
