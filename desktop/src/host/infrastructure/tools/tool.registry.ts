...
export class ToolRegistry {
  ...
  create(options: ToolRegistryOptions): ToolSet | undefined {
    ...
    const tools: ToolSet = {
      ...
      vecdb_search: tool({
        description:
          "Ищет релевантные фрагменты в разрешённых агенту векторных базах знаний. Результаты из нескольких хранилищ объединяются по рангу, а не по абсолютной оценке. scoreThreshold отсекает слабые совпадения внутри каждого хранилища по его собственной шкале.",
        inputSchema: z.object({
