...
interface SettingsProviderModelCardProps {
...
const formatSize = (bytes: number): string => {
  ...
  return `${(bytes / 1024 ** unitIndex).toLocaleString("ru-RU", { maximumFractionDigits: 1 })} ${units[unitIndex]}`;
};

export function SettingsProviderModelCard({
  model,
  enabled,
  onEnabledChange,
}: SettingsProviderModelCardProps) {
  const detail = [
