...
export function SettingsIntegrationsMailForm({
  ...
  return (
    <div className="grid grid-cols-2 gap-4 rounded-xl bg-main-800/35 p-4">
      ...
      <Field
        label={
          <ParameterLabel description="Порт IMAP-сервера. Для защищённого IMAP обычно используется 993.">
            Порт
          </ParameterLabel>
        }
      >
      ...
      <Field
        label={
          <ParameterLabel description="Секрет с паролем приложения или паролем почтового ящика. Сам пароль в профиле не хранится.">
            Пароль / app password
          </ParameterLabel>
        }
      >
      ...
      <Field
        label={
          <ParameterLabel description="Устанавливает зашифрованное TLS-соединение с IMAP-сервером. Рекомендуется не отключать.">
