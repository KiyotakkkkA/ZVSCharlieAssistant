    ...
    <div className="grid grid-cols-2 gap-4 rounded-xl bg-main-800/35 p-4">
      ...
      {connectionMetadata?.telegram ? (
        <div className="col-span-2 grid grid-cols-3 gap-3 rounded-xl bg-main-700/20 p-4">
          <BotInfo label="Бот" value={connectionMetadata.telegram.firstName ?? "Без имени"} />
          <BotInfo label="Имя пользователя" value={connectionMetadata.telegram.username ? `@${connectionMetadata.telegram.username}` : "Не задано"} />
          <BotInfo label="Telegram ID" value={String(connectionMetadata.telegram.id)} />
...
function BotInfo({ label, value }: { label: string; value: string }) {
  return <div><p className="text-[10px] text-main-500">{label}</p><p className="mt-1 truncate text-sm font-semibold text-main-100">{value}</p></div>;
}
