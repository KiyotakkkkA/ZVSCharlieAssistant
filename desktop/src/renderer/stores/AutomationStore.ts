...
class AutomationStore {
  ...
  get agents() {
    return this.agentStore.items;
  }
  get tools() {
    return this.toolStore.items;
  }
  get scenarios() {
    return this.scenarioStore.items;
  }
  get skills() {
    return this.skillStore.items;
  }
  ...
  async bootstrap(force = false) {
    ...
    this.error = null;
    try {
      const data = await window.desktop.automation.getSnapshot();
      runInAction(() => {
        this.agentStore.hydrate(data.agents);
        this.toolStore.hydrate(data.tools);
        this.scenarioStore.hydrate(data.scenarios);
        this.skillStore.hydrate(data.skills);
  ...
  getAgent = this.agentStore.get;
  getTool = this.toolStore.get;
  getScenario = this.scenarioStore.get;
  getSkill = this.skillStore.get;
  upsertAgent = this.agentStore.upsert;
  deleteAgent = this.agentStore.remove;
  upsertToolSecretBinding = this.toolStore.upsertSecretBinding;
  upsertScenario = this.scenarioStore.upsert;
  deleteScenario = this.scenarioStore.remove;
  validateScenario = this.scenarioStore.validate;
  startScenario = this.scenarioStore.start;
  approveScenarioRun = this.scenarioStore.approve;
  upsertSkill = this.skillStore.upsert;
  deleteSkill = this.skillStore.remove;
}
