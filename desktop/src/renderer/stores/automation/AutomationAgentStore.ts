...
import type { AutomationAgent, UpsertAutomationAgentInput } from "../../../ipc/contracts";

const payload = <T,>(value: T): T => JSON.parse(JSON.stringify(toJS(value))) as T;

export class AutomationAgentStore {
  items: AutomationAgent[] = [];
  constructor() { makeAutoObservable(this, {}, { autoBind: true }); }
  hydrate(items: AutomationAgent[]) { this.items = items; }
  ...
  async upsert(input: UpsertAutomationAgentInput) {
    ...
    runInAction(() => {
      ...
      if (index >= 0) this.items[index] = item; else this.items.unshift(item);
    });
    return item;
  }
