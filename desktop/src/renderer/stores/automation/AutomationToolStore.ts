...
import type { AutomationTool, UpsertAutomationToolSecretBindingInput } from "../../../ipc/contracts";
export class AutomationToolStore {
  items: AutomationTool[] = [];
  constructor() { makeAutoObservable(this, {}, { autoBind: true }); }
  hydrate(items: AutomationTool[]) { this.items = items; }
  ...
  async upsertSecretBinding(input: UpsertAutomationToolSecretBindingInput) {
    ...
    runInAction(() => { const index = this.items.findIndex(({ id }) => id === item.id); if (index >= 0) this.items[index] = item; });
    return item;
  }
}
