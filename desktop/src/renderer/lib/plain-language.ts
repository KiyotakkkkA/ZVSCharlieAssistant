...
export function humanizeError(error: unknown): string {
  ...
  if (/не найден текст|no text/i.test(raw))
    return "В документе не нашлось читаемого текста. Если это скан, возможно его качество слишком низкое, либо остсутствуют загруженные модели распознавания в разделе «Обработка».";
  return raw;
}
